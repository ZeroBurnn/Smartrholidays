...
def submit_quiz():
    try:
        ...
        if not all([name, phone, email, answers]):
            return jsonify({"status": "error", "message": "Заполните все поля"}), 400

        # Формируем сообщение для отправки в ВКонтакте
        ...
        # Отправляем сообщение в ВКонтакте
        vk_response = send_vk_message(name, phone, None, email)
        if not vk_response:
            return jsonify({"status": "error", "message": "Ошибка при отправке во ВКонтакте"}), 500
